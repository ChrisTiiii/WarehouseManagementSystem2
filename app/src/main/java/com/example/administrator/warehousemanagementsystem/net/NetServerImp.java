...
public class NetServerImp {
     ...
     * 同意审批
     */
    public void agreeReview(Integer reviewNo, Integer objId) {
        netAPI.agreeReview(myApp.user.getId(), reviewNo, objId, APP, myApp.getToken()).subscribeOn(Schedulers.io())//IO线程加载数据
                .observeOn(AndroidSchedulers.mainThread())//主线程显示数据
     ...
     * 拒绝请求
     */
    public void refuseReview(Integer reviewNo, String reason, Integer objId) {
        netAPI.refuseReview(myApp.user.getId(), reviewNo, reason, objId, APP, myApp.getToken()).subscribeOn(Schedulers.io())//IO线程加载数据
                .observeOn(AndroidSchedulers.mainThread())//主线程显示数据
                .subscribe(new Subscriber<BackData>() {
                    @Override
                    public void onCompleted() {
